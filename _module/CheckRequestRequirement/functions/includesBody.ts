...
export default function includesBody(request: Request, requiredBody: string[], strict: boolean = false) {
    try {
        ...
        if (strict && !onlyIncludesRequiredBody(body, requiredBody)) {
            throw new Error(`Only the following items are allowed for this route: [${requiredBody.join(", ")}]`);
        }
...
function includesRequiredBody(body: object, requiredBody: string[]) {
    return requiredBody.some((item: string) => Object.keys(body).includes(item));
}

function onlyIncludesRequiredBody(body: object, requiredBody: string[]) {
    return Object.keys(body).every((item: string) => requiredBody.includes(item));
}